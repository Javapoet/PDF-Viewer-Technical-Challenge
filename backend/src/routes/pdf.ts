...
console.log('routes/pdf.ts: __filename = ', __filename);
console.log('routes/pdf.ts: __dirname = ', __dirname);

const router = express.Router();

//const PDF_PATH = process.env.PDF_PATH || path.join(__dirname, '..', '..', 'assets', 'sample.pdf');
const PDF_NAME = 'Dermot-Doherty-Resume-Sept-2025.pdf';
const PDF_PATH = process.env.PDF_PATH || path.join(__dirname, '..', '..', 'assets', PDF_NAME);
console.log('routes/pdf.ts: PDF_PATH = ', PDF_PATH);
...
async function computeEtag(filePath: string): Promise<string> {
    console.log('routes/pdf.ts: computeEtag(filePath)', filePath);
...
router.get('/info', (_req, res) => {
    console.log('routes/pdf.ts: router.get(\'/info\', (_req, res)');
...
router.get('/stream', (req, res) => {
    console.log('routes/pdf.ts: router.stream(req, res)');
